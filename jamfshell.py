...
def get_uapi_token():
    ...
    jamf_hostname=input("Enter Jamf Hostname URL: (e.g. https://yourdomain.jamfcloud.com): ")
    ...
    if response.status_code == 200:
        ...
        print("Account error. Try again. Make sure you are using the correct credentials and URL.")
        get_uapi_token()

## handling warning for token timeout. Jamf I believe sets this to a half hour   
def timeout_handler(signal, frame):
    raise Exception('Jamf in a Shell Timeout Alert!')
signal.signal(signal.SIGALRM, timeout_handler)

def renewToken():
    renew_url = jamf_hostname + "/api/v1/auth/keep-alive"
    headers = {'Accept': 'application/json', 'Authorization': 'Bearer ' + api_token}
    response = requests.post(url=renew_url, headers=headers)
    if responseCheck(response) == "Success":
        print("Token renewed")
    else:
        print ("Token failed to renew. Uh Oh! Try logging back in again after quitting.")
...
def createStaticGroup(name, xmlCompList):
    url = jamf_hostname + "/JSSResource/computergroups/id/0"
    headers = {'Accept': 'application/xml', 'Authorization': 'Bearer ' + api_token}
    xml="<computer_group><name>" + name + "</name><is_smart>false</is_smart><id>-1</id>" \
        + xmlCompList + "</computer_group>"
...
def getGroupNameByID (groupID):
    return getGroupJson(groupID)['computer_group']['name']

def getSmartGroupCriteria(groupJson):
    if groupJson['computer_group']['is_smart'] == True:
        return groupJson['computer_group']['criteria']
    else:
        return

def extAttByGroup(query, printout):
    groupList=listAllGroups()
    results=[]
    for group in groupList:
        json=getGroupJson(group[1])
        if json['computer_group']['is_smart'] == True:
            criteria=getSmartGroupCriteria(json)
            if criteria != None:
                for criterium in criteria:
                    if query.lower() == criterium['name'].lower():
                ...
                groupName=group[0]
                groupID=group[1]
                print("Warning " + groupName + " has no criteria. ID: " + str(groupID))
    if printout == True:
        print(results)
    return results
...
def listComputersInGroup(groupID, parameter):
    compList=[]
    url = jamf_hostname + "/JSSResource/computergroups/id/" + str(groupID)
    headers = {'Accept': 'application/json', 'Authorization': 'Bearer ' + api_token}
    response = requests.request("GET", url, headers=headers)
    ...
    for i in range(0, len(response_json['computer_group']['computers'])):
        ...
        elif parameter == "SERIAL":
            compList.append(response_json['computer_group']['computers'][i]['serial_number'])
    return compList

def makeXMLComputerAdditionList(compList):
    xmlCompList="<computer_additions>"
    for computer in compList:
        xmlCompList = xmlCompList + "<computer><serial_number>" + computer + "</serial_number></computer>"
    xmlCompList = xmlCompList + "</computer_additions>"
    return xmlCompList

def smartGroupSnapshot(groupID):
    timestamp = datetime.now().strftime("%m/%d/%Y %H:%M:%S")
    name="static - " + getGroupNameByID(groupID) + ": Smart Group SnapShot @ " + timestamp
    ## get list of serials from smart group
    serialList=listComputersInGroup(groupID, "SERIAL")
    ## write list into computer additions xml format
    xmlList = makeXMLComputerAdditionList(serialList)
    ## create new static group and with list of computers, return response
    newGroup = createStaticGroup(name, xmlList)
    ## check response for failure
    if newGroup.status_code != 201:
        return "Error"
    else:
...
def groupAppUsage(groupID, appName, startDate, endDate):
    compList=listComputersInGroup(groupID, "ID")
...
def printCommandList ():
    ...
    ["smartgroup-snapshot", "Takes current computer list of Smart Group and creates a Static Group with that membership and timestamps it."],
    ...
    while loggedIn==True:
        try:       
            ...
            if command.lower() == "search":
                ...
                assetSearch(query)
            elif command.lower() == "smartgroup-snapshot":
                query=input("Enter Group ID of SmartGroup to Snapshot: ")
                name=getGroupNameByID(query)
                print("Retreiving current computer list from the group: " + name)
                snapshot=smartGroupSnapshot(query)
                if snapshot != "Error":
                    print("Static Group created.")
                else:
                    print("Error. Check IDs and try again?")
            ...
            renew=input('Your token is expiring. Would you like to renew? y/n: ')
            if renew.lower() == 'y':
                renewToken()
                signal.alarm(1680)
            else:
